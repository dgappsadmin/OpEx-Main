...
public class MonthlyMonitoringService {
    ...
    public MonthlyMonitoringEntry createMonitoringEntry(Long initiativeId, MonthlyMonitoringEntry monitoringEntry) {
        ...
        monitoringEntry.setInitiative(initiative);
        MonthlyMonitoringEntry savedEntry = monthlyMonitoringRepository.save(monitoringEntry);
        
        // Update Initiative's actual savings if entry is created as finalized
        if ("Y".equals(savedEntry.getIsFinalized())) {
            try {
                syncInitiativeActualSavings(initiativeId);
            } catch (Exception e) {
    ...
    public MonthlyMonitoringEntry updateMonitoringEntry(Long id, MonthlyMonitoringEntry entryDetails) {
        ...
        // Track if finalization status or achieved value changes
        boolean wasFinalizedBefore = "Y".equals(entry.getIsFinalized());
        BigDecimal previousAchievedValue = entry.getAchievedValue();
        ...
        // Update Initiative's actual savings if:
        // 1. Entry was finalized before or after the update, OR
        // 2. Achieved value changed for a finalized entry
        boolean isFinalizedAfter = "Y".equals(savedEntry.getIsFinalized());
        boolean achievedValueChanged = (previousAchievedValue == null && savedEntry.getAchievedValue() != null) ||
                                      (previousAchievedValue != null && !previousAchievedValue.equals(savedEntry.getAchievedValue()));
        
        if (wasFinalizedBefore || isFinalizedAfter || (isFinalizedAfter && achievedValueChanged)) {
            try {
                syncInitiativeActualSavings(entry.getInitiative().getId());
            } catch (Exception e) {
    ...
    public MonthlyMonitoringEntry updateFinalizationStatus(Long id, String isFinalized) {
        ...
        // Update Initiative's actual savings when entry is finalized or un-finalized
        try {
            syncInitiativeActualSavings(entry.getInitiative().getId());
        } catch (Exception e) {
    ...
    @Transactional
    public void deleteMonitoringEntry(Long id) {
        // Get the entry first to get the initiative ID
        Optional<MonthlyMonitoringEntry> entryOpt = monthlyMonitoringRepository.findById(id);
        if (entryOpt.isPresent()) {
            MonthlyMonitoringEntry entry = entryOpt.get();
            Long initiativeId = entry.getInitiative().getId();
            boolean wasFinalized = "Y".equals(entry.getIsFinalized());
            
            // Delete the entry
            monthlyMonitoringRepository.deleteById(id);
            
            // Update Initiative's actual savings if the deleted entry was finalized
            if (wasFinalized) {
                try {
                    syncInitiativeActualSavings(initiativeId);
                } catch (Exception e) {
    ...
    public List<MonthlyMonitoringEntry> batchFAApproval(List<Long> entryIds, String faComments) {
        ...
        // Note: actualSavings is now updated when entries are finalized, 
        // not when F&A approved, as per new requirement
