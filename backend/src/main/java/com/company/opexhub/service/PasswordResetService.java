...
public class PasswordResetService {
    ...
    public boolean sendResetCode(String email) {
        try {
            loggingService.info("Password reset requested - Email: " + email);
            
            // Check if user exists
            User user = userRepository.findByEmail(email).orElse(null);
            if (user == null) {
                loggingService.warning("Password reset failed - User not found: " + email);
                return false;
            }
            ...
            loggingService.info("Password reset code sent successfully - Email: " + email + " | User: " + user.getFullName());
            ...
            loggingService.error("Password reset email failed - Email: " + email, e);
            Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, 
                "Failed to send password reset email", e);
            return false;
        } catch (Exception e) {
            loggingService.error("Password reset code generation error - Email: " + email, e);
    ...
    public boolean verifyResetCode(String email, String code) {
        loggingService.info("Password reset code verification - Email: " + email);
        
        ResetCodeData resetData = resetCodes.get(email.toLowerCase());
        
        if (resetData == null) {
            loggingService.warning("Code verification failed - No code found for email: " + email);
            return false;
        }
        
        if (resetData.isExpired()) {
            loggingService.warning("Code verification failed - Code expired for email: " + email);
            resetCodes.remove(email.toLowerCase());
            return false;
        }
        
        boolean isValid = resetData.getCode().equals(code);
        if (isValid) {
            loggingService.info("Code verification successful - Email: " + email);
        } else {
            loggingService.warning("Code verification failed - Invalid code for email: " + email);
        }
    ...
    public boolean resetPassword(String email, String code, String newPassword) {
        loggingService.info("Password reset attempt - Email: " + email);
        
        // Verify code first
        if (!verifyResetCode(email, code)) {
            loggingService.warning("Password reset failed - Code verification failed for: " + email);
            return false;
        }
        ...
        try {
            ...
            User user = userRepository.findByEmail(email).orElse(null);
            if (user == null) {
                loggingService.error("Password reset failed - User not found: " + email);
                return false;
            }
            ...
            loggingService.info("Password reset successful - Email: " + email + " | User: " + user.getFullName());
            Logger.getLogger(this.getClass().getName()).info(
            ...
            loggingService.error("Password reset failed - Email: " + email, e);
            Logger.getLogger(this.getClass().getName()).log(Level.SEVERE, 
    ...
    public void cleanupExpiredCodes() {
        int beforeCount = resetCodes.size();
        resetCodes.entrySet().removeIf(entry -> entry.getValue().isExpired());
        int afterCount = resetCodes.size();
        
        if (beforeCount > afterCount) {
            loggingService.info("Password reset codes cleanup - Removed " + (beforeCount - afterCount) + " expired codes");
        }
    }
}