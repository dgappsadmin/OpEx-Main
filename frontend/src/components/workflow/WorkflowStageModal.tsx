...
export default function WorkflowStageModal({ 
  ...
  // Initiative Lead search state for stage 4
  const [ilSearchQuery, setIlSearchQuery] = useState('');
  const [showIlDropdown, setShowIlDropdown] = useState(false);
  const [selectedIlName, setSelectedIlName] = useState('');
  ...
  useEffect(() => {
    ...
    // Reset Initiative Lead search state when modal opens
    if (isOpen && transaction?.stageNumber === 4) {
      setIlSearchQuery('');
      setSelectedIlName('');
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (ilDropdownRef.current && !ilDropdownRef.current.contains(event.target as Node)) {
        setShowIlDropdown(false);
      }
    };
      ...
      case 4: // Define Responsibilities - CTSD assigns Initiative Lead (changed from EH to CTSD)
        return (
          <div className="space-y-4">
             ...
             */}
            <div className="relative" ref={ilDropdownRef}>
              <Label htmlFor="assignedUser" className="text-xs font-semibold">Select Initiative Lead *</Label>
              <div className="relative">
                <Input
                  id="assignedUser"
                  value={ilSearchQuery || selectedIlName}
                  onChange={(e) => {
                    setIlSearchQuery(e.target.value);
                    setShowIlDropdown(true);
                    // Clear selection if user types
                    if (e.target.value !== selectedIlName) {
                      setAssignedUserId('');
                      setSelectedIlName('');
                    }
                  }}
              ...
              {showIlDropdown && initiativeLeads.length > 0 && (
                <div className="absolute z-50 w-full mt-1 bg-white border border-gray-200 rounded-md shadow-lg max-h-40 overflow-y-auto">
                  {initiativeLeads
                    .filter(user => 
                      user.fullName?.toLowerCase().includes(ilSearchQuery.toLowerCase()) ||
                      user.email?.toLowerCase().includes(ilSearchQuery.toLowerCase()) ||
                      user.discipline?.toLowerCase().includes(ilSearchQuery.toLowerCase())
                    )
                    ...
                    .map((user) => (
                      <div
                        ...
                        onClick={() => {
                          ...
                          setSelectedIlName(user.fullName);
                          setIlSearchQuery(user.fullName);
                          setShowIlDropdown(false);
                        }}
                      >
